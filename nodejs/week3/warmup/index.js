...
app.get("/calculator/add", (req, res) => {
  try {
    ...
    isNaN(addition)
      ...
      : res.send(
          `Query Params are: ${queryParam} Addition is equal to: ${addition}`
        );
  } catch (error) {
    res.status(404).send("values undefined");
  }
});
...
app.get("/calculator/multiply", (req, res) => {
  try {
    const queryParam = Object.values(req.query);
    const data = queryParam.map(Number);
    const multiplication = data.reduce(
      (result, value) => (result = result * value),
      1
    );
    isNaN(multiplication)
      ? res.status(400).send("Invalid input")
      : res.send(
...
app.get("/calculator/divide", (req, res) => {
  try {
    ...
    isNaN(division)
      ? res.status(400).send("Invalid input")
      : res.send(
          `Query Params are: ${queryParam} Division is equal to: ${division}`
        );
  } catch (error) {
    res.status(404).send("values undefined");
  }
...
app.post("/calculator/multiply", (req, res) => {
  try {
    ...
    isNaN(division)
      ? res.status(400).send("Invalid input")
      : res.send(
          `Query Params are: ${queryParam} Multiplication is equal to: ${division}`
        );
  } catch (error) {
    res.status(404).send("values undefined");
  }
