...
UPDATE meal SET title = 'Ham cheese burger'
WHERE meal.id = 1;
...
SELECT title AS meal_title, description AS meal_description, 
price AS meal_price 
FROM meal
WHERE price < 90;
...
SUM(reservation.number_of_guests) AS reservation_guests, 
meal.max_reservations
FROM meal 
LEFT JOIN reservation ON reservation.meal_id = meal.id
GROUP BY reservation.meal_id
HAVING meal.max_reservations > reservation_guests or reservation_guests is null;
...
SELECT * 
FROM meal
WHERE created_date BETWEEN '2021-04-24' AND '2021-04-28';

-- Get only specific number of meals fx return only 5 meals

SELECT  count(id) AS number_of_meals 
FROM meal
WHERE price > 100;

-- Get the meals that have good reviews

SELECT  meal.id, meal.title, meal.description, meal.price, review.stars 
FROM meal
...
SELECT meal.id AS meal_id, meal.title AS meal_title, 
meal.description AS meal_description, review.stars 
FROM meal
JOIN review ON review.meal_id = meal.id
